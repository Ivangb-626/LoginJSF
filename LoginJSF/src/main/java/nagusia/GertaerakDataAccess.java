...
//lg5 = em.find(LoginGertaera.class, 1L, propietateak);
//System.out.println("5.1 => SELECT e FROM LoginGertaera e LEFT JOIN FETCH e.erabiltzailea WHERE e.id = :id");
//lg5 = em.createQuery("SELECT e FROM LoginGertaera e LEFT JOIN FETCH e.erabiltzailea WHERE e.id = :id",LoginGertaera.class)
//.setParameter("id", 1L)
//.getSingleResult();
//System.out.println("5.1 => SELECT DISTINCT e FROM LoginGertaera e LEFT JOIN FETCH e.erabiltzailea u LEFT JOIN FETCH u.gertaerak WHERE e.id = :id");
...
//.setParameter("id", 1L)
//.getSingleResult();
//System.out.println("5.2 => getErabiltzailea().getMota() => ");
...
//	}
	...
	em.persist(p1);
	em.getTransaction().commit();
	System.out.println("P:<" + e3.getPertsona() + "> E:<" +
	 e3.getPertsona().getErabiltzailea() + ">");
	} catch (Exception ex) {
	if (em.getTransaction().isActive()) {
	em.getTransaction().rollback();
	}
...
//		Query q = em.createQuery("delete from LoginGertaera where erabiltzailea = :erab");
...
//	 session.persist(e);
	...
	public boolean login(String izena, String pasahitza, String mota) {
		EntityManager em = JPAUtil.getEntityManager();
		boolean result = true;
		try {
		em.getTransaction().begin();
		TypedQuery<Erabiltzailea> q = em.createQuery("select e from Erabiltzailea e where e.izena= :erabiltzaileaIzena and e.pasahitza=:pasahitza and mota= :mota",Erabiltzailea.class);
		q.setParameter("erabiltzaileaIzena", izena);
		q.setParameter("pasahitza", pasahitza);
		q.setParameter("mota", mota);
		Erabiltzailea er = q.getSingleResult();
		em.getTransaction().commit();
